from MachineLearning.activation_functions import relu
...
class GradientDescent:
    ...
    def update_weights(weights, gradients, learning_rate):
        return weights - gradients * learning_rate

    @staticmethod
    def update_biases(biases, gradients, learning_rate):
        return biases - gradients * learning_rate
...
class MomentumGradientDescent:
    ...
    def update_weights(self, weights, gradients, learning_rate):
        self.velocity_w = self.momentum * self.velocity_w - learning_rate * gradients
        return weights + self.velocity_w

    def update_biases(self, biases, gradients, learning_rate):
        self.velocity_b = self.momentum * self.velocity_b - learning_rate * gradients
        return biases + self.velocity_b


class NeuralNetwork:
    def __init__(self, layers, learning_rate=0.1, gradient_descent=GradientDescent, gd_momentum=None):
        self.layers = layers
        self.learning_rate = learning_rate
        
        # Set gradient descent algorithm for each layer
        for layer in self.layers:
            layer.set_gradient_descent(gradient_descent, momentum=gd_momentum)
...
class Layer:
    ...
    def set_gradient_descent(self, gradient_descent, momentum=None):
        if gradient_descent == MomentumGradientDescent and momentum is not None:
            self.gd = MomentumGradientDescent(momentum=momentum)
        else:
...
class ConnectedLayer(Layer):
    ...
    def backward(self, output_derivatives, learning_rate):
        ...
        self.weights = self.gd.update_weights(self.weights, d_weights, learning_rate)
        self.biases = self.gd.update_biases(self.biases, undo_activation, learning_rate)
...
class CustomConnected(ConnectedLayer):
    ...
    def backward(self, output_derivatives, learning_rate):
        ...
        self.weights = self.gd.update_weights(self.weights, d_new_weights, learning_rate)
        self.biases = self.gd.update_biases(self.biases, undo_activation, learning_rate)
